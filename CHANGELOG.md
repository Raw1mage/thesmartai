...
- **SDK generation error**: Fixed `Function types cannot be represented in JSON Schema` error that prevented SDK regeneration. Removed `z.function().optional()` from `Command.Info` schema since `handler` is internal-only and doesn't need OpenAPI exposure.
- **Test failures in message-v2.test.ts**: Updated test expectations to match current AI SDK v5 output format:
  - Image attachments now use `{ type: "image", image: "data:..." }` instead of `{ type: "media", data, mediaType }`
  - Tool output always uses `{ type: "content", value: [...] }` wrapper format
  - Step-start boundaries no longer split assistant messages
- **Test failures in provider.test.ts**: Updated `enabled_providers` test to use `disabled_providers` (blacklist) since whitelist functionality was removed from implementation. Skipped legacy `enabled_providers` tests.

### Changed

- **Provider filtering simplified**: `enabled_providers` (whitelist) is no longer supported in `Provider.list()`. Use `disabled_providers` (blacklist) instead. UI show/hide is handled separately in local storage.

### Deployed

- **Production build**: Version `0.0.0-cms-202602012325` deployed to Docker container `opencode-web` on port 1080.
