...
| `src/server/routes/workspace.ts`    | **Workspace Routes.** API boundary for workspace list/current/status/read backed by runtime workspace service.                | `WorkspaceRoutes`    | **In:** Instance project/workspace<br>**Out:** Workspace list/read/status |
...
| `src/project/workspace/*`         | **Workspace Kernel (Phase 1).** Defines workspace aggregate types, directory→workspace resolution, attachment ownership contracts, registry interface, and a runtime workspace service façade that now registers session/PTY attachments and serves project workspace summaries. | `resolveWorkspace`, `WorkspaceRegistry`, `WorkspaceService`, workspace schemas/types | **In:** Project info + directory + session/pty events<br>**Out:** Workspace aggregate / registry/service lookups |
