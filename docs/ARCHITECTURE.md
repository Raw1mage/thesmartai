...
| `src/project/workspace/*`         | **Workspace Kernel (Phase 1).** Defines workspace aggregate types, directory→workspace resolution, attachment ownership contracts, registry interface, and a runtime workspace service façade for future workspace-scoped evolution. | `resolveWorkspace`, `WorkspaceRegistry`, `WorkspaceService`, workspace schemas/types | **In:** Project info + directory<br>**Out:** Workspace aggregate / registry/service lookups |
