...
| `src/project/workspace/*`         | **Workspace Kernel (Phase 1).** Defines workspace aggregate types, directory→workspace resolution, attachment ownership contracts, registry interface, and a runtime workspace service façade that now begins registering session/PTY attachments. | `resolveWorkspace`, `WorkspaceRegistry`, `WorkspaceService`, workspace schemas/types | **In:** Project info + directory + session/pty events<br>**Out:** Workspace aggregate / registry/service lookups |
...
| `context/global-sync/child-store.ts`              | **Workspace Adapter Bridge.** Maintains directory-scoped child stores and now derives app-visible `workspace` identity snapshots from `project + worktree + directory`.                       | `createChildStoreManager`           | **In**: Directory + hydrated project/path state<br>**Out**: Child store + workspace snapshot |
