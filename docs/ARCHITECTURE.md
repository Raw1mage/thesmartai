...
| `src/server/routes/file.ts`         | **File Routes.** API for file reading, searching, and git status; `file.status` is the canonical current-workspace git-uncommitted file source for UI `Changes`/`檔案異動` surfaces, and emits review diagnostic headers for web observability.                                                                                                              | `FileRoutes`         | **In:** Path/Pattern<br>**Out:** Content/List                                                 |
...
Current review/diff ownership now follows a split contract: runtime owns both plain git working-tree truth and optional session attribution, while app/TUI own presentation and local caching behavior.
...
3. **Canonical UI `Changes` source is current workspace git-uncommitted state**
   - current API surface: `file.status`
   - semantics: all current uncommitted files for the resolved workspace directory
   - TUI / webapp `Changes` / `檔案異動` / dirty count surfaces should consume this source for operator-facing review/commit awareness

4. **Session-owned attribution remains a separate runtime contract**
   ...
   - this is attribution/provenance data, not the default UI `Changes` truth
