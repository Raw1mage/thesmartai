...
| `src/server/routes/workspace.ts`    | **Workspace Routes.** API boundary for workspace list/current/status/read plus minimal lifecycle state transitions backed by runtime workspace service. | `WorkspaceRoutes`    | **In:** Instance project/workspace<br>**Out:** Workspace list/read/status/lifecycle |
...
| `src/project/workspace/*`         | **Workspace Kernel (Phase 1).** Defines workspace aggregate types, directory→workspace resolution, attachment ownership contracts, registry interface, runtime workspace service façade, minimal lifecycle transition contract, worker attachment registration, and `workspace.*` bus events for aggregate observers. `previewIds` remains a reserved field until a real preview runtime domain/event source exists. | `resolveWorkspace`, `WorkspaceRegistry`, `WorkspaceService`, `WorkspaceEvent`, workspace schemas/types | **In:** Project info + directory + session/pty/worker events<br>**Out:** Workspace aggregate / registry/service/lifecycle/event lookups |
...
| `context/global-sync/child-store.ts`              | **Workspace Adapter Bridge.** Maintains directory-scoped child stores and now derives app-visible `workspace` identity snapshots from `project + worktree + directory`, with bootstrap state also carrying runtime workspace status summaries. | `createChildStoreManager`                     | **In**: Directory + hydrated project/path state<br>**Out**: Child store + workspace snapshot/status |
| `context/global-sync/bootstrap.ts`                | **Workspace-Aware Directory Bootstrap.** Bootstraps per-directory state and now consumes runtime `/workspace/current` + `/workspace/status` via authenticated low-level fetch until SDK regeneration catches up.                               | `bootstrapDirectory`, workspace fetch helpers | **In**: SDK client + auth fetch + directory<br>**Out**: Bootstrapped workspace-aware child state    |
...
| `pages/layout.tsx`                                | **Workspace UX Orchestrator.** Sidebar project/workspace interactions, prefetch/navigation logic, and now lifecycle transition calls for reset/delete flows routed through runtime `/workspace/:id/*` endpoints.                               | `Page`                                        | **In**: globalSync + globalSDK + route state<br>**Out**: Sidebar/workspace UX                       |
