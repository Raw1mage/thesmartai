...
#### 功能：更新 gemini-cli 可選模型清單

**需求**

- gemini-cli 可選模型僅保留以下 5 個：
  - gemini-3-pro-preview
  - gemini-3-flash-preview
  - gemini-2.5-pro
  - gemini-2.5-flash
  - gemini-2.5-flash-lite
- 後端 provider 清單與前端顯示一致。
- 修正後跑一次 `bun run typecheck`。

**範圍**

- IN: gemini-cli provider 模型清單、UI/CLI 顯示清單、預設候選模型。
- OUT: 其他 provider、非必要重構。

**作法**

1. 更新 gemini-cli provider 的模型定義為 5 個指定模型。
2. 更新 CLI/前端的 gemini-cli 可選清單與預設候選。
3. 執行 typecheck 驗證。

**任務**

1. [x] 更新 gemini-cli provider 模型清單。
2. [x] 更新 CLI/前端可選模型與預設候選。
3. [x] 執行 typecheck。
