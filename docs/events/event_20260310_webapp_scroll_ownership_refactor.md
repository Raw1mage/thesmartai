...
### 最終狀態（2026-03-13 使用者確認）

- **follow-bottom 模式**：✅ 完美。無縫捲動，無震盪、無閃爍。
- **free-reading 模式**：⚠️ iOS anchor 觸發時仍會被強制跳位。此為 WebKit 已知行為，JS 層無法防護（momentum scroll 與 anchor jump 不可區分）。目前策略：free-reading 時不做任何校正，接受偶發跳位。

### 已知限制

1. **free-reading 模式下 iOS anchor jump 無法防護**：momentum scroll 與 anchor jump 不可區分，任何校正都會干擾正常慣性滑動。
2. **Circuit breaker 為安全網**：若 follow-bottom 遇到高頻跳錨，3 次/秒即熔斷，需使用者手動按追底鈕恢復。實測中熔斷未觸發（追底機制本身已足夠穩定）。
...
| `2d1d6aa061` | Circuit breaker 擴展至 ResizeObserver snap 路徑（修復結論 streaming 震盪） |
