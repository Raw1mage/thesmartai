...
describe("prompt-input history", () => {
  ...
  test("canNavigateHistoryAtCursor only allows input boundaries", () => {
    const value = "a\nb\nc"
    ...
    expect(canNavigateHistoryAtCursor("up", value, 2, true)).toBe(false)
    expect(canNavigateHistoryAtCursor("down", value, 2, true)).toBe(false)
    expect(canNavigateHistoryAtCursor("up", value, 0, true)).toBe(true)
    expect(canNavigateHistoryAtCursor("down", value, value.length, true)).toBe(true)
  })
})
