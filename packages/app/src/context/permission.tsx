...
export const { use: usePermission, provider: PermissionProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    function isAutoAccepting(sessionID: string, directory?: string) {
      ...
      return autoRespondsPermission(store.autoAcceptEdits, session, { sessionID }, directory)
    }

    function isAutoAcceptingDirectory(directory: string) {
      return isDirectoryAutoAccepting(store.autoAcceptEdits, directory)
    }

    function shouldAutoRespond(permission: PermissionRequest, directory?: string) {
      const session = directory ? globalSync.child(directory, { bootstrap: false })[0].session : []
      return autoRespondsPermission(store.autoAcceptEdits, session, permission, directory)
    }
    ...
    function disableDirectory(directory: string) {
      ...
      setStore(
        produce((draft) => {
          draft.autoAcceptEdits[key] = false
        }),
      )
    }
    ...
    return {
      ...
      autoResponds(permission: PermissionRequest, directory?: string) {
        return shouldAutoRespond(permission, directory) && shouldAutoAccept(permission)
      },
      isAutoAccepting,
