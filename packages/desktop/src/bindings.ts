...
export const commands = {
  ...
  getDisplayBackend: () =>
    __TAURI_INVOKE<{ phase: "server_waiting" } | { phase: "sqlite_waiting" } | { phase: "done" } | null>(
      "get_display_backend",
    ),
  setDisplayBackend: (backend: InitStep) => __TAURI_INVOKE<null>("set_display_backend", { backend }),
  parseMarkdownCommand: (markdown: string) => __TAURI_INVOKE<string>("parse_markdown_command", { markdown }),
  checkAppExists: (appName: string) => __TAURI_INVOKE<boolean>("check_app_exists", { appName }),
  wslPath: (path: string, mode: { phase: "server_waiting" } | { phase: "sqlite_waiting" } | { phase: "done" } | null) =>
    __TAURI_INVOKE<string>("wsl_path", { path, mode }),
