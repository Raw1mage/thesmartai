...
export function renderModelCheckReport(report: ProviderHealth.HealthReport): string {
    ...
    for (const [index, account] of report.accounts.entries()) {
      ...
      if (account.models && account.models.length > 0 && account.modelsWorking > 0) {
        ...
        for (const model of account.models) {
          ...
          const responseTime =
            model.responseTime !== undefined && model.responseTime !== null ? `${model.responseTime}ms` : "-"
  ...
  lines.push("---")
  lines.push(
    "💡 *Tip: Models showing green status are ready for use. Use `opencode allocate-models` to update your routing based on this report.*",
  )
