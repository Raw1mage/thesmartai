...
const activeGoogleAppIds = mock(async () => [])
...
describe("MCP startup Google token sweep when no active Google apps exist", () => {
  afterEach(() => {
    sweepSharedGoogleAccessToken.mockClear()
    activeGoogleAppIds.mockReset()
    activeGoogleAppIds.mockResolvedValue([])
    subscribe.mockClear()
    publish.mockClear()
  })

  it("reports that startup sweep should not run", async () => {
    const { MCP } = await import("./index")

    await expect(MCP.shouldRunSharedGoogleStartupSweep()).resolves.toBeFalse()
    expect(activeGoogleAppIds).toHaveBeenCalledTimes(1)
    expect(sweepSharedGoogleAccessToken).toHaveBeenCalledTimes(0)
  })
})
