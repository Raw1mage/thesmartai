...
export class ProactiveRefreshQueue {
   ...
   */
  private async refreshToken(auth: OAuthAuthDetails, account: ManagedAccount): Promise<OAuthAuthDetails | undefined> {
    const minutesUntilExpiry = account.expires ? Math.round((account.expires - Date.now()) / 60000) : "unknown"
