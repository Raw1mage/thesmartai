...
describe("extractThinkingConfig", () => {
  ...
  it("converts Anthropic-style thinking config", () => {
    const result = extractThinkingConfig({ thinking: { type: "enabled", budgetTokens: 10000 } }, undefined, undefined)
...
describe("filterUnsignedThinkingBlocks", () => {
  ...
  it("preserves tool blocks even if they have signature-like fields", () => {
    const contents = [
      {
        role: "user",
        parts: [{ type: "tool_result", tool_use_id: "tool_456", content: "result", signature: "some_random_value" }],
      },
    ]
    const result = filterUnsignedThinkingBlocks(contents)
    expect(result[0].parts).toHaveLength(1)
...
describe("filterMessagesThinkingBlocks", () => {
  ...
  it("preserves non-thinking blocks and returns message unchanged when content is missing", () => {
    const messages: any[] = [{ role: "assistant", content: [{ type: "text", text: "hello" }] }, { role: "assistant" }]
  ...
  it("converts Gemini-style thoughtSignature to providerMetadata.anthropic.signature", () => {
    const response = {
      candidates: [
        {
          content: {
            parts: [{ thought: true, text: "thinking here", thoughtSignature: "sig123abc" }, { text: "output" }],
          },
        },
      ],
    }
  ...
  it("converts Anthropic-style signature to providerMetadata.anthropic.signature", () => {
    const response = {
      candidates: [
        {
          content: {
            parts: [{ type: "thinking", text: "thinking here", signature: "anthro_sig_xyz" }, { text: "output" }],
          },
        },
      ],
    }
  ...
  it("prefers signature over thoughtSignature when both present", () => {
    const response = {
      candidates: [
        {
          content: {
            parts: [{ thought: true, text: "thinking", signature: "sig_primary", thoughtSignature: "sig_fallback" }],
          },
        },
      ],
    }
  ...
  it("preserves non-thinking parts unchanged", () => {
    ...
    const chunk = {
      candidates: [
        {
          content: {
            parts: [{ thought: true, text: "Thinking" }, { text: "Regular text" }, { functionCall: { name: "test" } }],
          },
        },
      ],
    }
