  ...
  return contents.map((content: any, idx: number) => {
    ...
    if (Array.isArray((content as any).content)) {
      ...
      const isLastAssistantContent =
        idx === lastAssistantIdx || (isAssistantRole && idx === findLastAssistantIndex(contents, "assistant"))
...
export function deepFilterThinkingBlocks(
  ...
  const walk = (value: unknown): void => {
    ...
    if (Array.isArray(obj.contents)) {
      obj.contents = filterUnsignedThinkingBlocks(obj.contents as any[], sessionId, getCachedSignatureFn, isClaudeModel)
    }

    if (Array.isArray(obj.messages)) {
      obj.messages = filterMessagesThinkingBlocks(obj.messages as any[], sessionId, getCachedSignatureFn, isClaudeModel)
    }
