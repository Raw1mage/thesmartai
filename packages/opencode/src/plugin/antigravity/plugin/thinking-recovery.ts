...
function messageHasThinking(msg: any): boolean {
  ...
  if (Array.isArray(msg.content)) {
    return msg.content.some((block: any) => block?.type === "thinking" || block?.type === "redacted_thinking")
  }
...
export function looksLikeCompactedThinkingTurn(msg: any): boolean {
  ...
  const hasThinking = parts.some(
    (p: any) =>
      p && typeof p === "object" && (p.thought === true || p.type === "thinking" || p.type === "redacted_thinking"),
  )
