 ...
 */
export function ensureClaudeMaxOutputTokens(generationConfig: Record<string, unknown>, thinkingBudget: number): void {
...
export function normalizeClaudeTools(
  ...
  ;(payload.tools as unknown[]).forEach((tool: unknown) => {
    ...
    if (t.function || t.custom || t.parameters || t.input_schema || t.inputSchema) {
      pushDeclaration(
        (t.function as Record<string, unknown> | undefined) ?? (t.custom as Record<string, unknown> | undefined) ?? t,
        "function/custom",
      )
      return
    }
 ...
 */
export function applyClaudeTransforms(payload: RequestPayload, options: ClaudeTransformOptions): ClaudeTransformResult {
