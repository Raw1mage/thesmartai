...
async function collect<T>(iter: AsyncGenerator<T>) {
  const result: T[] = []
...
describe("Session.listGlobal", () => {
  test("lists sessions across projects with project metadata", async () => {
    ...
    const sessions = await collect(Session.listGlobal({ limit: 200 }))
    ...
    const firstProject = projects.find((project) => project.id === firstSession.projectID)
    const secondProject = projects.find((project) => project.id === secondSession.projectID)
  ...
  test("excludes archived sessions by default", async () => {
    ...
    await Instance.provide({
      directory: tmp.path,
      fn: async () =>
        Session.update(archived.id, (draft) => {
    ...
    const allSessions = await collect(Session.listGlobal({ limit: 200, archived: true }))
  ...
  test("supports cursor pagination", async () => {
    ...
    const page = await collect(Session.listGlobal({ directory: tmp.path, limit: 1 }))
    expect(page.length).toBe(1)
    expect(page[0].id).toBe(second.id)

    const next = await collect(Session.listGlobal({ directory: tmp.path, limit: 10, cursor: page[0].time.updated }))
