...
function normalizeOpen(input: unknown): string[] {
  if (typeof input === "function") {
    try {
      const next = (input as (current: string[]) => unknown)([])
      return normalizeOpen(next)
    } catch {
      return []
    }
  }
  if (Array.isArray(input)) return input.filter((value): value is string => typeof value === "string")
  if (input instanceof Set) return [...input].filter((value): value is string => typeof value === "string")
...
export const SessionReview = (props: SessionReviewProps) => {
  ...
  const open = () => normalizeOpen(props.open ?? store.open)
  ...
  const handleChange = (next: unknown) => {
    ...
    const nextOpen =
      typeof next === "function"
        ? normalizeOpen((next as (value: string[]) => unknown)(currentOpen))
        : normalizeOpen(next)
  ...
  return (
    ...
    >
      ...
      >
        <Show when={props.title}>{(title) => <div data-slot="session-review-title">{title()}</div>}</Show>
            ...
            <For each={props.diffs}>
              {(diff) => {
                ...
                return (
                  ...
                  >
                    <div data-slot="session-review-item-header">
                      <button
                        type="button"
                        data-slot="session-review-trigger-button"
                        onClick={() => {
                          const current = open()
                          if (current[0] === diff.file) {
                            handleChange([])
                            return
                          }
                          handleChange([diff.file])
                        }}
                      >
                        <div data-slot="session-review-trigger-content">
                          <div data-slot="session-review-file-info">
                            <Show
                              ...
                              fallback={<FileIcon node={{ path: diff.file, type: "file" }} />}
                            >
                              <button
                                type="button"
                                data-slot="session-review-file-icon-button"
                                onClick={(e) => {
                                  e.stopPropagation()
                            ...
                            <div data-slot="session-review-file-name-container" title={diff.file}>
                              <span data-slot="session-review-filepath">{`\u202A${diff.file}\u202C`}</span>
                            </div>
                          </div>
