...
- [x] 1.2 Define and implement the phased `ApplyPatchMetadata` shape and backward-compatibility rules
...
- [x] 4.1 Validate multi-file running expandability and phase visibility (`bun test` passed, phased metadata test added; live TUI interaction still pending manual verification)
- [x] 4.2 Validate completed diff/diagnostics compatibility and failed-state behavior (code-path inspection completed; package-local typecheck remains blocked by unrelated existing errors in `task-worker-continuation.ts` / `session/prompt.ts`)
