...
// Fix return type
code = code.replace('export async function set(providerId: string, info: Info): Promise<string> {', 'export async function set(providerId: string, info: Info): Promise<string | undefined> {');
code = code.replace('      // short hash of token (ultimate fallback — always unique)', '      // short hash of token (ultimate fallback — always unique)\n');

// Add fallthrough return
const oldOauthEnd = `        })
        return accountId
      }
    }
...
const newOauthEnd = `        })
  ...
  /**
   * Remove auth for a provider`;
code = code.replace(oldOauthEnd, newOauthEnd);

// Revert bogus Provider.dispose
const oldRemove = `  export async function remove(providerId: string) {
...
const newRemove = `  export async function remove(providerId: string) {
    const { Account } = await import("../account")
