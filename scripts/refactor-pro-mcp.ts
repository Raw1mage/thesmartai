...
const SERVER_NAME = "refactor-pro"
const SERVER_VERSION = "0.2.0"
const CHARACTER_LIMIT = 120_000
const ROOT = process.env["REFACTOR_PRO_ROOT"] || process.cwd()
...
server.registerTool(
  ...
  {
    title: "Refactor Pro Daily Delta",
    description:
...
server.registerTool(
  ...
  {
    title: "Refactor Pro Generate Plan",
    description:
...
server.registerTool(
  ...
  {
    title: "Refactor Pro Update Ledger",
    description:
...
server.registerTool(
  ...
  {
    title: "Refactor Pro Skill Index",
    description:
...
server.registerTool(
  ...
  {
    title: "Refactor Pro Wizard Hint",
    description:
  ...
  async ({ phase, sourceRef, targetRef }) => {
    const steps: Record<string, string[]> = {
      analysis: [
        `1) Run refactor_pro_daily_delta on ${targetRef}..${sourceRef}`,
        "2) Load refactor-from-src skill via refactor_pro_skill_read",
        "3) Review high-risk commits in protected paths first",
      ],
      planning: [
        "1) Run refactor_pro_generate_plan",
      ...
      execution: [
        "1) Use refactor_pro_wizard_execute to guide user",
        "2) For each commit: refactor_pro_wizard_question asks user to preview/apply/skip",
        "3) Validate lint + typecheck + focused tests after each integration",
      ],
      ledger: [
        "1) Collect final statuses",
        "2) Run refactor_pro_update_ledger",
...
// New wizard execution tools
server.registerTool(
  "refactor_pro_wizard_question",
  {
    title: "Refactor Pro Wizard Question",
    description:
      "Interactive wizard question system for guiding user through refactor decisions. Presents options and waits for user selection.",
    inputSchema: z
      .object({
        question: z.string().min(1).describe("Question to ask the user"),
        context: z.string().optional().describe("Additional context or commit details"),
        options: z
          .array(
            z.object({
              label: z.string(),
        ...
        defaultOption: z.string().optional().describe("Default selected option"),
      })
      .strict(),
    annotations: {
      readOnlyHint: true,
  ...
  async ({ question, context, options, defaultOption }) => {
    const result = {
      question,
      context: context || "",
      options,
      defaultOption,
      note: "This tool presents a question to the user. The agent should call agent-workflow skill and use Question tool to gather user input.",
    }
    return {
      content: [
        {
...
server.registerTool(
  ...
  {
    title: "Refactor Pro Wizard Execute",
    description:
      "Interactive wizard mode that guides user through executing refactor plan. For each integrated/ported commit, presents options: preview diff, apply changes, skip, or abort. Integrates with agent-workflow skill.",
    inputSchema: z
      .object({
        planPath: z.string().min(1).describe("Path to refactor plan markdown"),
        ledgerPath: z.string().min(1).describe("Path to commit ledger"),
        mode: z.enum(["preview", "execute"]).default("preview").describe("preview=dry run, execute=apply changes"),
        commitHash: z.string().optional().describe("Specific commit hash to process (if not provided, processes all)"),
      })
      .strict(),
    annotations: {
      readOnlyHint: false,
  ...
  async ({ planPath, ledgerPath, mode, commitHash }) => {
    const abs = normalizePath(planPath)
    const planText = await readFile(abs, "utf8")

    // Parse plan to extract commits marked as integrated/ported
    const commitLines = planText.match(/\|\s*`([0-9a-f]{7,9})`\s*\|.*\|\s*(integrated|ported)\s*\|/gi) || []
    const commits = commitLines
      .map((line) => {
        const match = line.match(/`([0-9a-f]{7,9})`\s*\|.*\|\s*(integrated|ported)/)
        return match ? { hash: match[1], action: match[2] } : null
      })
      .filter(Boolean) as Array<{ hash: string; action: string }>

    const filtered = commitHash ? commits.filter((c) => c.hash.startsWith(commitHash)) : commits

    if (filtered.length === 0) {
      return {
        content: [{ type: "text", text: `No commits found to process${commitHash ? ` matching ${commitHash}` : ""}.` }],
        structuredContent: { commits: [], mode, planPath, ledgerPath },
      }
    }
      ...
      nextSteps:
        mode === "preview"
          ? [
              ...
              "2. For each commit, use refactor_pro_wizard_question to ask user: [Preview Diff / Apply / Skip / Abort]",
          ...
          : [
              "1. Agent will execute actions for each commit",
              "2. Low-risk: attempt git cherry-pick",
              "3. High-risk: show diff and wait for manual port confirmation",
              "4. Update ledger after each successful integration",
            ],
    }
    ...
    return {
      content: [
        {
          type: "text",
          text: `Wizard Execute (${mode} mode)\nPlan: ${abs}\nFound ${filtered.length} commits to process:\n${filtered.map((c, i) => `${i + 1}. ${c.hash} (${c.action})`).join("\n")}\n\nNext: Agent should use refactor_pro_wizard_question to guide user through each commit.`,
        },
      ],
      structuredContent: result,
    }
...
async function main() {
  if (Bun.argv.includes("--help")) {
    ...
    console.log("Usage:")
    console.log(`  bun scripts/refactor-pro-mcp.ts`)
    console.log("Environment:")
    console.log("  REFACTOR_PRO_ROOT=/absolute/repo/path (optional)")
    return
  }
