...
- 此切片當時引用的 `packages/opencode/src/session/prompt/runner.txt` 已於後續 runtime hardening 中移除；現行 autonomous execution wording 已回收到 `packages/opencode/src/session/workflow-runner.ts`。
