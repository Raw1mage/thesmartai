...
- Run `bun run ~/projects/skills/plan-builder/scripts/plan-sync.ts specs/_archive/autonomous-opt-in/` after every checkbox toggle; honor drift warnings per §16.3 decision tree
...
- [ ] `plan-sync.ts specs/_archive/autonomous-opt-in/` currently reports `clean` (so any drift detected during implementation is attributable to the current work)
