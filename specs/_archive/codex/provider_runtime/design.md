...
處置：完整刪除 C library 與 build 系統（見 `specs/_archive/codex/revision/2026-04-18_codex-c-library-removal/`）。codex provider 正式定位為純 TS package；未來若需要 native path，需重新開 spec，不可悄悄補回 C sources。
...
- `specs/_archive/codex/protocol/whitepaper.md` 是 source-derived interoperability note。
- `specs/_archive/codex/provider_runtime/` 是 opencode 自己的正式實作策略。
