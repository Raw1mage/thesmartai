...
Parent: `specs/_archive/codex/provider_runtime/` (living). 本 revision 純清理；沒有新功能、沒有協議改動。`provider_runtime/design.md` 原本並未宣告 codex 走 native FFI，因此這次刪除只是讓 runtime 實況與 spec 描述一致。
...
- **DD-5 Doc 同步只補 cross-reference，不做大改.** `specs/_archive/codex/provider_runtime/design.md` 加一條 `DD-N: codex C library 於 2026-04-18 刪除（見 revision/2026-04-18_codex-c-library-removal）`。`plans/codex-refactor/plan.md` 提到 C library 的段落加「REMOVED 2026-04-18」註記，不 rewrite 整個 plan。
...
- Doc update:
  - `specs/_archive/codex/provider_runtime/design.md` — 補 cross-reference DD
