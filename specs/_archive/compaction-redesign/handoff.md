...
3. Use `bun run ~/projects/skills/plan-builder/scripts/plan-sync.ts
   specs/_archive/compaction-redesign/` after every task checkbox toggle.
...
- `specs/_archive/compaction-redesign/proposal.md` — Why / Scope / Design Decisions DD-1..DD-10
- `specs/_archive/compaction-redesign/spec.md` — R-1..R-9 with GIVEN/WHEN/THEN scenarios
- `specs/_archive/compaction-redesign/design.md` — state-driven algorithm, KIND_CHAIN + INJECT_CONTINUE tables, critical files, risks
- `specs/_archive/compaction-redesign/data-schema.json` — type contracts (SessionMemory, TurnSummary, Anchor, RunInput)
- `specs/_archive/compaction-redesign/c4.json` — component map; especially the `removed` array for deleted concepts
- `specs/_archive/compaction-redesign/sequence.json` — 7 runtime scenarios that the build must reproduce in tests
- `specs/_archive/compaction-redesign/test-vectors.json` — concrete input/expected pairs per requirement
...
- [ ] `bun run ~/projects/skills/plan-builder/scripts/plan-state.ts specs/_archive/compaction-redesign/` returns `state: planned`
- [ ] `bun run ~/projects/skills/plan-builder/scripts/plan-validate.ts specs/_archive/compaction-redesign/` returns PASS for all required artifacts
