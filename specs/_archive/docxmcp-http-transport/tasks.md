...
- [x] 1.1 在 `bin/_token_store.py` 實作 `TokenStore` class：内存 dict {token → {path, sha, mtime, size}}；`create()` 用 `secrets.token_hex` + base32 編碼產 `tok_<32-char>`（DD-3）
- [x] 1.2 `TokenStore.put(file_stream, filename)` — streaming 寫到 `/tmp/docxmcp-sessions/<token>/<filename>`，回傳 `{token, sha256, size}`（R2-S1, DD-11）
- [x] 1.3 `TokenStore.resolve(token) -> Path` — 回傳檔案 path；找不到 raise `TokenNotFoundError`（R3-S2）
- [x] 1.4 `TokenStore.delete(token)` — 刪 fs + 移除 entry（R5-S2）
- [x] 1.5 TTL 60 分鐘 + 1GB 總體 cap + LRU eviction 背景 task（threading.Timer 每 60s 檢查，DD-4）
- [x] 1.6 `bin/mcp_server.py` 加 starlette routes：
...
- [x] 1.7 unit tests `tests/test_token_store.py`：put/resolve/delete/TTL/LRU 覆蓋；測 50MB streaming 不爆記憶體

## 2. Tool schema rewrite — 21 wrappers from path → token (docxmcp Python)

- [x] 2.1 在 `bin/_mcp_registry.py` 加 `_token_arg_schema()` helper 回傳 `{type:"object", properties:{token:{type:"string", pattern:"^tok_[A-Z2-7]{32}$"}, ...}, required:["token", ...]}`
- [x] 2.2 對 21 支 ToolSpec 一一改寫：
...
- [x] 2.3 加 token-not-found 統一錯誤路徑：catch `TokenNotFoundError` 回 `{ isError: true, content: [{type:"text", text:"token_not_found: <tok>; container restarted, please re-upload"}] }`（R3-S2）
- [x] 2.4 unit tests `tests/test_tool_registry_token.py`：每支 tool 用 fake token 跑通 + token 失效時錯誤訊息正確

## 3. Container side — Dockerfile + compose (docxmcp)

- [x] 3.1 `Dockerfile` 移除 `/state` 假設；確認 `/tmp/docxmcp-sessions/` 是 ephemeral；`EXPOSE 8080` 保留；加 `HEALTHCHECK CMD curl -f http://localhost:8080/healthz || exit 1`
- [x] 3.2 `docker-compose.yml`：
...
- [x] 3.4 啟動後 `curl --unix-socket /run/user/$UID/opencode/sockets/docxmcp/docxmcp.sock http://localhost/healthz` 200
- [x] 3.5 多 user 並存驗證：cece 也跑 `docker compose -p docxmcp-cece up -d`，確認兩 container 並存、socket 各自隔離（R1-S3 / R10-S2 / AC-19）

## 4. Non-Docker surface — bin-wrappers + install.sh (docxmcp)

- [x] 4.1 在 `bin-wrappers/` 寫 21 支 shell 腳本，模板：
  ```bash
  #!/usr/bin/env bash
  set -euo pipefail
  host_path="$1"; shift
...
- [x] 4.2 所有 wrapper 加 docxmcp container 存活預檢：`docker inspect "$container" >/dev/null 2>&1 || { echo "docxmcp container not running; docker compose up -d" >&2; exit 1; }`（RK-6）
- [x] 4.3 寫 `install.sh`：
...
- [x] 4.4 manual test：跑 `extract_text /home/pkcs12/projects/opencode/incoming/合約.docx`，驗 stdout 對 + 無新建 mount（AC-11 / R6-S1）

## 5. opencode mcp client — transport switch + lint (opencode)

- [x] 5.1 在 `packages/opencode/src/mcp/manifest.ts` Manifest schema 加 `transport: z.enum(["stdio", "streamable-http", "sse"]).optional().default("stdio")` + `url: z.string().url().optional()`
- [x] 5.2 在 `packages/opencode/src/mcp/index.ts` 加 transport switch：manifest.transport 為 `streamable-http` 時用 `StreamableHTTPClientTransport`，URL 支援 `unix:///<sock-path>:<http-path>` 形式（解析出 socketPath + http path、傳給 fetch via Bun `unix:` 機制 / undici socketPath）；其它仍 `StdioClientTransport`（DD-8, DD-12, DD-16 / R4-S1, R4-S2）
- [x] 5.3 在 `packages/opencode/src/mcp/app-store.ts` `addApp()` 加 lint：
  ...
  - 不符例外 → throw `BindMountForbidden` 含 `policy: "specs/_archive/docxmcp-http-transport"`、`evidence: [...]`（R8-S1）
  - 同樣 lint 也對 `cloneAndRegister` 有效
- [x] 5.4 在 `packages/opencode/src/server/routes/mcp.ts` 加 `GET /api/v2/mcp/store/audit-bind-mounts`：掃所有 entries 回 violations 陣列（R8-S2）
- [x] 5.5 unit tests：lint 偵測 `-v /a:/b` / `--mount type=bind,src=...` / `-v` 在不同位置；audit endpoint 回正確結構

## 6. opencode dispatcher — HTTP uploader rewrite (opencode)

- [x] 6.1 `packages/opencode/src/incoming/dispatcher.ts` **大刪**：
...
- [x] 6.2 改寫 `before(toolName, args, appId, sessionID)`：
...
- [x] 6.3 寫 `uploadToDocxmcp(repoPath, projectRoot)` helper：
...
- [x] 6.5 寫 `deleteToken(token)` helper — `DELETE /files/{token}`，吞 error
- [x] 6.6 移除 `packages/opencode/src/incoming/index.ts` 中 `maybeBreakIncomingHardLink` export（不再需要）
- [x] 6.7 移除 `packages/opencode/src/tool/edit.ts` `tool/write.ts` 中 `maybeBreakIncomingHardLink` 呼叫（hard-link 不存在了）
- [x] 6.8 既有 dispatcher tests 砍 hard-link / break-on-write / cross-fs 相關 cases；新增 HTTP uploader cases（mock fetch）

## 7. mcp-apps.json 切換 + cutover (cross-repo)

- [x] 7.1 docxmcp `mcp.json` 改寫為 URL form：`{ id:"docxmcp", url:"http://127.0.0.1:8080/mcp", transport:"streamable-http", ... }` （R7-S1）
- [x] 7.2 git tag `pre-http-transport-cutover` 在 docxmcp + opencode 兩 repo（RK-8）
- [x] 7.3 撤 docxmcp 從現有 mcp-apps.json：`DELETE /api/v2/mcp/store/apps/docxmcp`
- [x] 7.4 重新註冊：`POST /api/v2/mcp/store/apps {path:"/home/pkcs12/projects/docxmcp", target:"user"}` — manifest 改寫後新 entry 應為 URL form
- [x] 7.5 daemon 重啟（**需使用者明示同意**，呼叫 `system-manager:restart_self` MCP tool）
- [x] 7.6 確認 docxmcp container `docker inspect` Mounts 為空（除 named volume）（AC-01）
- [x] 7.7 確認 daemon 連 docxmcp 走 HTTP transport，21 tools 出現在 tool list（AC-07）

## 8. End-to-end smoke

- [x] 8.1 上傳一個 docx：驗 `<projectRoot>/incoming/<file>` 出現、history jsonl 寫入（R9-S1）
- [x] 8.2 AI 觸發 `docx_decompose(incoming/<file>)`：
...
- [x] 8.3 同 session 再次呼叫 — 新 token 但 sha 命中容器內 cache（DD-5），mcp 回 `from_cache:true`
- [x] 8.4 跨 session 跨 project（同 sha）— named volume cache 仍命中
- [x] 8.5 docker restart docxmcp container — token 全失效；下次工具呼叫 dispatcher 重新 upload，從 named volume cache 命中（cache 跨重啟）
- [x] 8.6 AC-13 驗證：`grep -r 'home/' ~/.config/opencode/mcp-apps.json` 應為空
- [x] 8.7 AC-14 驗證：手動 POST 新 entry 含 `-v /tmp:/x` → 收到 400 + `bind_mount_forbidden`
- [x] 8.8 AC-15 驗證：`GET /api/v2/mcp/store/audit-bind-mounts` 回 `{violations:[], totalEntries:N}`（已是空）
- [x] 8.9 wrapper 驗證：`extract_text /tmp/test.docx` 跑通；`docker exec` 期間無新 mount

## 9. Sunset bind-mount in repo-incoming-attachments + docs sync

- [ ] 9.1 `specs/_archive/repo-incoming-attachments/design.md` 對 DD-3/5/11/15/16 做 inline-delta SUPERSEDED 標記，指向 `specs/_archive/docxmcp-http-transport/`（OQ-7 收斂為 inline-delta，不走 revise mode 避免狀態機複雜）
...
- [ ] 10.3 `bun run scripts/plan-sync.ts specs/_archive/docxmcp-http-transport/` 跑一次最終 sync，無 drift
- [ ] 10.4 promote `verified`：`bun run scripts/plan-promote.ts specs/_archive/docxmcp-http-transport/ --to verified --reason "all 17 ACs pass; manual smoke OK; bind-mount audit empty"`
