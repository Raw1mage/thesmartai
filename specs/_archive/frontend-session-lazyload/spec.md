...
> **SUPERSEDED 2026-04-24**: SSE replay 機制整個拆掉。SSE 現在只發 live events，斷線時缺口事件直接丟掉，clients 靠 user scroll-up 或離開重進 route 恢復。細節見 `specs/_archive/mobile-tail-first-simplification/` DD-3 + R2。下方原始 R8 內容保留作為歷史紀錄，**不再為現行 runtime contract**。
...
> **SUPERSEDED 2026-04-24**: `beforeMessageID` cursor query param 改名為 canonical `before`。Cold-open tail-first 行為保留（由 `session_tail_mobile` / `session_tail_desktop` 控制），但 R9 所描述的 `beforeMessageID` 協議已不存在。細節見 `specs/_archive/mobile-tail-first-simplification/` R1 + R3。下方原始 R9 內容保留作為歷史紀錄，**不再為現行 runtime contract**。
