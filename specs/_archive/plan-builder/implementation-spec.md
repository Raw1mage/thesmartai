...
- **Phase 5 驗收**：dog-fooding migration 實測失敗（本 plan 搬進 `specs/_archive/plan-builder/` 過程出錯），停下並回歸 Phase 2 修補
...
- `/home/pkcs12/projects/opencode/plans/plan-builder/`（本 dog-fooding 計畫自身位置，最終會 migrate 到 `specs/_archive/plan-builder/`）
...
- **Phase 5 — Dog-fooding migration 驗證**：以本 plan（`plans/plan-builder/`）當第一個 migration 測試案例，執行 `ensureNewFormat()` 讓它搬到 `specs/_archive/plan-builder/` + 快照到 `.archive/pre-migration/`，驗證狀態推斷結果、history 寫入、git history 保留。
...
- **Phase 2 驗收**：
  - `bun run scripts/plan-state.ts specs/_archive/plan-builder/` 正確印出當前 state
  - `bun run scripts/plan-validate.ts specs/_archive/plan-builder/` 只驗當前 state 要求的 artifact 且通過
...
- **Phase 5 驗收**：
  - 本 `plans/plan-builder/` 自動 migrate 到 `specs/_archive/plan-builder/`
  - `specs/_archive/plan-builder/.state.json` 存在且 state 推斷為 `implementing`（因為 tasks.md 含已勾選項）
  - `specs/_archive/plan-builder/.archive/pre-migration/` 快照存在
  - `git log --follow specs/_archive/plan-builder/proposal.md` 可追溯到 `plans/plan-builder/proposal.md` 的 history
