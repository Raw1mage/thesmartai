...
- **THEN** 必須自動搬到 `specs/_archive/plan-builder/`，狀態推斷為 `implementing`（因 tasks.md 有勾選項），快照保留於 `.archive/pre-migration-YYYYMMDD/`
...
- `git log --follow` 可追溯本 `plans/plan-builder/proposal.md` 至 migrate 後的 `specs/_archive/plan-builder/proposal.md`
...
- 本 plan 經過完整 Phase 1–6 後，`specs/_archive/plan-builder/.state.json.state = "verified"` 且 history 含完整七次轉換紀錄
