...
- [ ] 5.2 驗證自動 migration：`plans/plan-builder/` 已不存在，`specs/_archive/plan-builder/` 存在且含 `.state.json`
- [ ] 5.3 驗證 `.state.json.state = "implementing"`（因 tasks.md 有已勾選項）
- [ ] 5.4 驗證 `specs/_archive/plan-builder/.archive/pre-migration-YYYYMMDD/` 快照存在
- [ ] 5.5 驗證 `git log --follow specs/_archive/plan-builder/proposal.md` 可追溯到 `plans/plan-builder/proposal.md`
