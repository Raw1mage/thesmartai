...
### ~~DD-3：mcp 容器邊界 vs 搬運責任~~ (v1, SUPERSEDED 2026-05-03 → `specs/_archive/docxmcp-http-transport/`)
...
args) is documented in `specs/_archive/docxmcp-http-transport/design.md` DD-1,
...
### ~~DD-5：staging 區走通用結構~~ (v1, SUPERSEDED 2026-05-03 → `specs/_archive/docxmcp-http-transport/`)
...
### ~~DD-11：cross-session bundle 用 hard-link + break-on-write 包裝~~ (v1, SUPERSEDED 2026-05-03 → `specs/_archive/docxmcp-http-transport/`)
...
### ~~DD-15：跨 fs hard-link EXDEV 自動退回 cp~~ (v1, SUPERSEDED 2026-05-03 → `specs/_archive/docxmcp-http-transport/`)
...
### ~~DD-16：cache integrity 透過 docxmcp-side `manifest.json` 驗證~~ (v1, SUPERSEDED 2026-05-03 → `specs/_archive/docxmcp-http-transport/`)
