...
- [x] 6.2 docxmcp repo `HANDOVER.md`「不要重新討論」清單中「Bundle 預設落點：XDG_STATE + by-session」標 `[SUPERSEDED 2026-05-03 → opencode/specs/_archive/repo-incoming-attachments/]`
- [x] 6.2.1 docxmcp repo `HANDOVER.md`「不要重新討論」新增一條：「每個 bundle 必含 `manifest.json`，schema 見 opencode/specs/_archive/repo-incoming-attachments/data-schema.json#BundleManifest，至少 sha256/appId/appVersion/createdAt」(DD-16 跨 repo contract)
...
- [ ] 7.3 `bun run scripts/plan-sync.ts specs/_archive/repo-incoming-attachments/` 跑一次最終 sync，確認沒 drift warning
- [ ] 7.4 promote `verified`：`bun run scripts/plan-promote.ts specs/_archive/repo-incoming-attachments/ --to verified --reason "all 15 ACs pass; manual smoke OK"`
