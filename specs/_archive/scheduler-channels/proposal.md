...
2. 長時間 downtime 後的 stale schedule 必須採用 skip-to-next / disable-on-expiry 等 fail-fast recovery 策略
3. Durable scheduler baseline is already implemented and must be treated as current-state behavior, not future design only
4. Real daemon startup must wire scheduler lifecycle correctly so run-log / execution evidence is produced on the live unix-socket path

### Planned Runtime-Control-Plane Extensions

5. SystemEvents queue 在 restart 後要能重建或安全清空（acceptable loss）
6. Daemon mode 支援多個 channel（獨立 agent 對話），每個 channel 有獨立的 session scope
7. Channel 之間有 lane isolation — 一個 channel 的 agent turn 不會擠壓另一個 channel 的 concurrency
8. Kill-switch 可以按 channel scope 或 global scope 觸發
