...
- Durable cron scheduling in OpenCode has evolved across multiple artifacts: the older `specs/_archive/scheduler-channels/` package, recent runtime fixes in cron code, and two 2026-03-27 debug events that proved minute-level cadence and daemon lifecycle wiring both mattered.
...
- Treat `specs/_archive/scheduler-channels/` as historical architecture context, but treat 2026-03-27 event evidence as the current-state baseline for scheduler durability behavior.
