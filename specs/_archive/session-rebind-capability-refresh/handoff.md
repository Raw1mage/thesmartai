...
6. **Sync after each task** — 每次 `- [x]` 後跑 `bun run ~/projects/skills/plan-builder/scripts/plan-sync.ts specs/_archive/session-rebind-capability-refresh/`。
...
- `specs/_archive/mandatory-skills-preload/design.md` → DD-1..DD-10 了解上一階段決策脈絡
...
1. 立刻 `- [x]` in `specs/_archive/session-rebind-capability-refresh/tasks.md`
2. `bun run ~/projects/skills/plan-builder/scripts/plan-sync.ts specs/_archive/session-rebind-capability-refresh/`
3. 讀 sync output：
   - `clean` → 繼續
