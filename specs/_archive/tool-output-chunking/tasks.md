...
- [x] 2.1 Move `specs/_archive/tool-output-chunking/hybrid-llm-framing.md` → `packages/opencode/src/session/prompt/hybrid-llm-framing.md` (`git mv` to preserve history)
...
- [x] 2.7 chunk-and-merge mode (DD-3 internal mode) — runLlmCompactChunkAndMerge dispatches when input > LLM input budget
...
- [x] 2.10 Phase 2 absorb-pinned-zone path (DD-5/DD-9) — fires when Phase 1 attempts exhaust AND pinned_zone non-empty
- [x] 2.11 Phase 2 starvation E_OVERFLOW_UNRECOVERABLE (INV-6) — bounded chain length 2; no Phase 3
...
- [x] 2.17 Add fixtures TV-3..TV-7, TV-10..TV-16, TV-20 to `test-vectors.json` and write `bun test` runners
...
- [x] 2.22 Update `specs/architecture.md` Compaction Subsystem section (rewrite for hybrid-llm)
- [x] 2.23 Phase 2 summary: write `docs/events/event_<YYYYMMDD>_hybrid-llm-landing.md`
