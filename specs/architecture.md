...
- `packages/opencode/src/session/prompt.ts` now applies a minimal committed planner intent guard: if the latest assistant narration explicitly committed to `plan_exit`, later round-boundary routing must suppress opposite-direction `plan_enter` auto-routing.
