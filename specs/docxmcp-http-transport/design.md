...
**Decision**：
- **撤除**：`/specs/repo-incoming-attachments` 的 DD-3 (mcp /state mount)、DD-5 (staging dir)、DD-11 (hard-link + break-on-write)、DD-15 (EXDEV fallback)、DD-16 (host-side manifest sha integrity)
...
- `specs/repo-incoming-attachments/design.md` DD-3/5/11/15/16 inline-delta SUPERSEDED → 指向本 spec
