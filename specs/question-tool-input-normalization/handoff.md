...
- 每打勾一個 task 就跑 `bun run ~/projects/skills/plan-builder/scripts/plan-sync.ts specs/question-tool-input-normalization/`
- 每跑完一個 Phase 就更新 `specs/question-tool-input-normalization/handoff.md` 的 Progress 段（或寫入 event log 的 phase-summary block）
- 不動 `specs/question-tool-abort-fix/` 的任何檔案（scope 明確區分）
...
- [ ] `specs/question-tool-abort-fix/` 狀態是 `living`，不會被本次工作改動
