...
### DD-11：cross-session bundle 用 hard-link + break-on-write 包裝
...
### DD-17：AttachmentBlob schema 演進為「content 退場 + repoPath 新增」(2026-05-03，supersedes DD-10)

**Decision**：`SessionStorage.AttachmentBlob` 結構演進如下：
...
interface AttachmentBlob {
  ...
  // Legacy field — present on rows written before this spec landed.
  // New rows MUST NOT set this; readers MUST still accept it.
  content?: Uint8Array

  // New fields (this spec). New rows MUST set both. Legacy rows do not have them.
  repoPath?: string   // relative to session.project.worktree, always under "incoming/"
  sha256?: string     // sha of the file at repoPath at write time
}
```

`getAttachmentBlob(sessionID, refID)` 行為：
- 若 row 含 `repoPath`：read bytes from `<repo>/<repoPath>`、回傳含 fresh `content` 的 AttachmentBlob。檔案不存在 → 明確報錯，不退回任何 cache（no-silent-fallback rule）。
- 若 row 缺 `repoPath` 但含 `content`：直接回 `content`（legacy path）。
- 兩者都缺：報錯「malformed AttachmentBlob row」。

`upsertAttachmentBlob` 寫入時：本 spec 之後的所有新 row 必須含 `repoPath` + `sha256`、`content` 留空（`undefined`）。

**Rationale**：
- 用 optional 欄位演進 schema 比 discriminated union 容易遷移、不需改 LegacyStore / SqliteStore 內部。
- 舊 row 永遠可讀；不主動遷移；不浪費 daemon 啟動時間掃舊資料。
- 真正解的問題：bytes 從 session storage 拉回專案 + 跨 session 不再 base64 重複塞。

**Migration**：不遷移、不清理。舊 row 跟著 session 一起終老。

**Risk**：daemon 升級後若回滾，舊 daemon 會看到含 `repoPath` 的 row 但讀不懂 → 退回試讀 `content`（不存在）→ 拋錯。Mitigation：新 daemon 推上前先 grace period（一個 release）讓舊 daemon 也學會 forward-compat 讀 `repoPath`，但實際 read 仍走舊 path。本 spec 範圍不展開此 grace period 設計，留作 implementing 階段細節。
...
### DD-15：跨 fs hard-link EXDEV 自動退回 cp
...
### DD-16：cache integrity 透過 docxmcp-side `manifest.json` 驗證
