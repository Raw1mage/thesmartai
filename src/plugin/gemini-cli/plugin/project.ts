...
function buildIneligibleTierMessage(tiers?: GeminiIneligibleTier[]): string | undefined {
  ...
  const reasons = tiers.map((tier) => tier?.reasonMessage?.trim()).filter((message): message is string => !!message)
  if (reasons.length === 0) {
    return undefined
  }
  return reasons.join(", ")
...
export async function ensureProjectContext(
  ...
  const resolveContext = async (): Promise<ProjectContextResult> =>
    resolveProjectContextFromAccessToken(auth, accessToken, configuredProjectId, async (updatedAuth) => {
