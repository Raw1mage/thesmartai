...
do_sync_config() {
    ...
    if [ -x "./scripts/sync-config.sh" ]; then
...
do_deploy() {
    ...
    echo ""
    echo "To view logs:   ./webctl.sh logs"
    echo "To check status: ./webctl.sh status"
    echo ""
}
...
do_status() {
    ...
    if docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
        ...
        echo ""
        echo "  Run './webctl.sh start' to start the service"
    fi
    echo ""
}
...
do_help() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0 start                           # Start on port 1080"
    echo "  OPENCODE_PORT=8080 $0 start        # Start on port 8080"
    echo "  OPENCODE_SERVER_PASSWORD=secret $0 deploy  # Deploy with password"
    echo ""
}
